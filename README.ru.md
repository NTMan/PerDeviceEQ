...
![Per-Device EQ — Основное окно](per-device-eq-main.png)

![Per-Device EQ — Окно измерений](per-device-eq-measurements.png)
...
- **Искажения в протоколе.** Каждый тейк хранит свои гармоники и
  собственный шум; они рисуются по тейкам и сводятся в
  полосу-предупреждение под коррекцией, так что подъём, который будет
  слышен как хрип, виден до того, как рука к нему потянется.
...
2. **Объявите его каналы захвата.** Нажмите **+** на полосе: у рига
   ровно те каналы, которые объявила рука, и никаких других. У каждого
   свой файл **калибровки** (*Change…*), а полоска на его вкладке живая —
   стукните по капсюлю, и шевельнётся та вкладка, куда он пришёл; так
   каналы различают, ничего не перетыкая.
3. **Откалибруйте чувствительность.** Кнопка рядом с фейдером усиления
   ...
   под фейдером, и шаг пропускается.
4. **Выберите выход** в *Output* — куда играет свип.
5. **Объявите измеряемые каналы.** **+** предлагает каналы синка, а
   затем — какой капсюль каждый из них пишет. Когда в работе один
   капсюль, отвечать нечего: он пишет все цели, и под его вкладкой
   напечатано, какие именно. Когда их два, в подменю у каждого капсюля
   живой уровень, так что ответ — стук и клик.
6. **Откалибруйте громкость.** Первое **play** на риге, которого
   приложение ещё не видело, само охотится за уровнем пробными свипами:
   достаточно горячим для чистого тейка и безопасно далёким от клипа, а
   если оба условия несовместимы — честно отказывает. Это одно число на
   весь риг, а не число на канал; фейдер показывает его, а
   кнопка рядом забывает запомненное и меряет здесь и сейчас.
7. **Наберите примерно три чистых тейка на канал.** Чистый помечен
   зелёной точкой. Пересаживайте устройство между тейками: разброс между
...
профиль уже играет. Плашка доверия под графиком предложит **Re-fit**,
когда вы позже добавите или удалите тейки.
...
которого просят ход, которого у него нет: там, где добавка баса покупает
не бас, а искажения. Его ручка несёт полосу-предупреждение по верху
собственной шкалы: одна кривая на весь профиль, худший канал на каждой
частоте, крашенная там, где измеренные искажения переходят порог
слышимости. Порог движется с частотой, потому что движется слух —
примерно −18 дБ относительно тона на 20 Гц против −40 дБ от килогерца и
выше. В басу целые проценты проходят незамеченными, в середине не
проходят и десятые. Профиль, тейки которого не несут признания, не
красится вовсе: отсутствие данных и отсутствие искажений не должны
носить один цвет.
...
- **Ручная доводка:** тяните точку на графике, клик по пустому месту
  ...
  правильно.
- **Поканальный эквалайзер:** у каждого канала выхода своя вкладка и своя
  кривая. Профиль с одним каналом -- это одна кривая, и она играет на
  всех каналах.
