...
Поиск уровня меряет, где это начинается. Найдя уровень для тейков, он
идёт ВВЕРХ, начиная заметно ниже него, шагами по 2 дБ, сравнивает
каждую ступень с самой тихой и кладёт найденное в профиль: на каждую
ступень — уровень, пик захвата и всю кривую. Три-четыре лишних свипа.
...
* **Полоса уровня над карточкой устройства** дублирует системную
  ...
  значило бы гадать.
* **Зелёная кривая краснеет там, где риг не поспевает**, и меняет имя
  на *what you hear* — этот участок уже измерение, а не прогноз.
...
* **На внутриканальных она живёт до следующего надевания.** Три обхода
  одного наушника здесь разошлись на 1.5 дБ ниже 30 Гц при нуле на
  килогерце — подпись посадки, а не драйвера.
* **Выше самой громкой ступени она молчит**, если обход остановил
  захват, а не риг. Это и есть серый участок, и на трёх из пяти
  здешних ригов обход кончился именно так.
* **Каждая ступень — один свип.** По Bluetooth один из них пришёл на
  0.78 дБ ниже на нескольких десятках герц, чего не видели два других
  прогона. На проводных такого не встречалось.
